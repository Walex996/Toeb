...
$FinalTargetDatabase = 'Alabagbe'
$OctoOptionDeployToLocalInstance = ''
$SkipVariableValidation = ''
$__isAzurePlatformTarget = 'False'
$CreateScriptFileName = 'Toeb.Database_Package.sql'
$SnapshotPackageFileName = 'Toeb.Database_Snapshot.nupkg.bin'
...
function Read-Snapshot($ScriptDirectory) {
  try{
    ...
    Write-Warning "Failed to read snapshot from file: $_"
    return $null
  }
}
function Write-Snapshot($HexString) {
  $WriteFailedMessage = "No snapshot will be written to the target database. Reports will be unavailable for the next deployment."
  ...
  $ReadyRollSnapshotTableName = "__SchemaSnapshot"
  ...
  try {
    ...
    Write-Warning "Failed to write snapshot to database: $_"
    Write-Warning $WriteFailedMessage
  }
  finally{
    $SqlConnection.Close()
...
Write-Output "Reading snapshot"
$HexString = Read-Snapshot(Get-ScriptDirectory)

Write-Output "Writing snapshot to target database"
