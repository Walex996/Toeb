...
interface JQueryStatic {
    ...
    cookie(token: string, accessToken?: Object,other?:any): any;
    bootstrapGrowl(val, p: { ele: string;type;offset: { from;amount: number };align;width: number;delay;allow_dismiss: boolean;stackup_spacing: number }): any;
    notific8(trim?: string, settings?: any,zindex?:any): any;
    sessionTimeout(p: { title: string;message: string;keepAliveUrl: string;redirUrl: string;logoutUrl: string;warnAfter: number;redirAfter: number }): any;
    idleTimeout:any;
    
    gritter;
    removeCookie(token: string, p: { path: string }): any
    Notification;
    notify(p: { icon: string; message: string; url: string }, p1: { element: string; type: string; allow_dismiss: boolean; newest_on_top: boolean; showProgressbar: boolean; placement: { from: string; align: string }; offset: number; spacing: number; z_index: number; delay: number; timer: number; animate: { enter: string; exit: string } });
    validator;
}
...
interface JQuery {
    ...
    */
    ...
    parsley(): any;
    summernote(p: { height: number; focus: boolean }): any;
    printElement
    waitMe(p:any): any;
    pagination(object: Object, p: { callback: () => any;num_display_entries: number;num_edge_entries: number;items_per_page: number }): any;
    code(item): any;
    validate: any;
    modal(show: string): any;
    wysihtml5():any;
    datepicker: any;
    bootstrapWizard
    valid(): any;
    rules(add: string, p: { required: boolean }): any;
    steps(p:any): any;
    gmap
    highcharts(p: { chart: { type: string;options3d: { enabled: boolean;alpha: number;beta: number } };title: { text: string };tooltip: { pointFormat: string };plotOptions: { pie: { allowPointSelect: boolean;cursor: string;depth: number;dataLabels: { enabled: boolean;format: string } } };series: any[] });
    slimscroll(p: { height: string; wheelStep: number });
    bootstrapSwitch;
    multiSelect;
    timepicker;
    niceScroll;
